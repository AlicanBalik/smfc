...
function IosPublisher() {
	...
	this.startPublish = function(config, data) {
		...
		iosConfigOutput = iosConfig.output;
	...
	function createSmartfaceVersionFile() {
		iosPlayerZip.zip.folder(DEST_FOLDER).file('SmartfaceVersion.txt', '{"runtimeVersion":"' + config2.fileVersion + '", "SASVersion":"' + iosConfig.sasVersion + '"}');
