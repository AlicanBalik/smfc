...
var ERROR = require('../../interactive/error');
...
function commandGenerator(identifier_string, lambda) {
	return function() {
		...
		if (!utility.controlChildProcess(child)) {
			err.err = 'child error';
			var errorApktool = ERROR.apkTool[identifier_string];
			if (!errorApktool) {
				err.msg = ERROR.signer;
			} else {
				err.msg = errorApktool;
			}
			err.detail = child.output.toString('utf8');
