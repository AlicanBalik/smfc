...
exports.throwsNoSuchDir = function(dir) {
	...
	if (!safeControlDirectory(dir)) {
		...
		err.fullPath = convertAbsolute.convert(dir);
		killProcess(err);
	}
}
...
exports.throwsNoSuchFile = function(file, type) {
	...
	if (!safeControlFile(file, type)) {
		...
		err.fullPath = convertAbsolute.convert(file);
		killProcess(err);
	}
}
...
function writeJsonToStdout(jsonObj) {
	if (LogStatus.getOutJson()) {
		...
		if (jsonObj.err) {
			console.log('ERROR: ' + jsonObj.err);
			console.log(' └──── ' + jsonObj.msg);
			if (jsonObj.fullPath) {
				console.log(' Full Path: ' + jsonObj.fullPath);
			}
			if (jsonObj.detail) {
				console.log(' Detail: ' + jsonObj.detail);
			}
		...
		if (jsonObj.responseDone) {
			console.log('Output: ' + savedData.downloadFilePath);
		}
	}
}
...
exports.convertAbsolute = convertAbsolute;
...
function pngImagesControl(imagesDir, pkcgProfiles) {
	if (!fs.existsSync(imagesDir)) {
		killProcess({
			...
			msg: 'Image folder not exists. : ' + imagesDir
		});
	}
...
function findSfpxAndDataFolder(_input) {
	var parsedFile = path.parse(_input);
