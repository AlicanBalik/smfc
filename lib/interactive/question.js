...
function validatePlayer(_input, infoStr, fileExt) {
	var res = false;
...
function askPlayerArm(callback) {
	...
	function validateArm(_input) {
		_input = _input.trim();
		return validatePlayer(_input, QUESTIONS.playerArm.description, 'apk');
	}
}
...
function askPlayerx86(callback) {
	...
	function validatex86(_input) {
		_input = _input.trim();
		return validatePlayer(_input, QUESTIONS.playerx86.description, 'apk');
	}
}

function askPlayeriOS(callback) {
	UI.wait(QUESTIONS.playeriOS.description);
	var opt = {
		validate: validateiOS,
		filter: filterAbsolutePath
	}
	asker(QUESTIONS.playeriOS, callback, {
		midObj: 'iOS',
		opt: opt
	});
	...
	function validateiOS(_input) {
		_input = _input.trim();
		return validatePlayer(_input, QUESTIONS.playeriOS.description, 'zip');
	}
}
...
function askForiOS(callback) {
	askPlayeriOS(function() {
		UI.complete(QUESTIONS.playeriOS.description);
		askLicense(function() {
			UI.complete(QUESTIONS.license.description);
			callback();
		});
