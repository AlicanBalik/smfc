...
function checkForBasicParams(_args) {
	if (!_args.projectRoot) {
		killProcess({
			err: 'Basic Parameters',
			msg: 'You must pass this parameter : projectRoot \n If you want more information of projectRoot, run : smfc --help=projectRoot '
		});
	}
}
...
function parseForWin32(data, argv) {
	...
	function commonFullPublish() {
		var root = convert(argv.projectRoot) || undefined;
		return {
...
function parseForC9(data, argv) {
	...
	if (task === 'android-full-publish') {
		...
		data.moduleGlobals.tmpdir = outputTempFolder;
		_.extend(args, {
			PLAYER_ZIP: argv.inputZip || BIN_ROOT + '/iOS_Player.zip',
	...
	function loadConfigJson(argv) {
		...
		try {
			...
			var errMsg = "config_file could not be found : " + path.normalize(config_file_path);
			logger.fatal(errMsg);
			killProcess({
				err: "Config File",
				msg: errMsg
