...
function checkForArgsError(args) {
	...
	if (indexOfUndef !== -1) {
		var err = {
			err: 'parser error',
			msg: 'invalid argument detected ! check parameters for --> ' + arg_keys[indexOfUndef]
		};
...
function parseForWin32(data, argv) {
	...
	convertAbsolute.setCwd(data.cwd);
	...
	if (task === 'android-full-publish') {
		args = _.extend(commonFullPublish(), {
			JAVA: convertAbsolute.convert(argv.java) || 'java',
			APK_TOOL: convertAbsolute.convert(argv.apkTool) || 'bin/apktool.jar',
			INPUT_APK: convertAbsolute.convert(argv.inputApk) || null,
			INPUT_APK_x86: convertAbsolute.convert(argv.inputApkx86) || null,
			...
			OUTPUT_APK: convertAbsolute.convert(argv.outputApk) || 'Smartface Demo.apk',
			SIGN_OUTPUT_APK: null,
			SIGNER: convertAbsolute.convert(argv.signer) || 'bin/SignApk.jar',
			KEYSTORE_FILE: convertAbsolute.convert(argv.keystoreFile) || 'test-files/input/smfdefault.keystore',
		...
		args = _.extend(commonFullPublish(), {
			PLAYER_ZIP: convertAbsolute.convert(argv.inputZip) || undefined,
			OUTPUT_ZIP: convertAbsolute.convert(argv.outputZip) || 'Smartface Demo.zip'
		});
	}
	...
	function commonFullPublish() {
		var root = convertAbsolute.convert(argv.projectRoot) || undefined;
		return {
			PROJECT_SFPX: convertAbsolute.convert(argv.sfpx) || undefined,
			LICENSE_FILEPATH: convertAbsolute.convert(argv.licenseFile) || 'test-files/input/data2.sfd',
			projectRoot: root,
...
function parseForC9(data, argv) {
	var args;
	var task = argv.task;
	argv.projectRoot = utility.convertAbsolute.convert(argv.projectRoot);
	...
	if (task === 'android-full-publish') {
		...
		_.extend(args, {
			JAVA: argv.java || 'java',
			APK_TOOL: argv.apkTool || BIN_ROOT + '/apktool.jar',
			outputTempFolder: outputTempFolder,
			INPUT_APK: argv.inputApk || BIN_ROOT + '/SmartfacePlayer.apk',
			INPUT_APK_x86: argv.inputApkx86 || null,
			DECOMP_INPUT_APK: null,
			...
			SIGN_OUTPUT_APK: null,
			SIGNER: argv.signer || BIN_ROOT + '/SignApk.jar',
			KEYSTORE_FILE: root + '/' + config_signer.keystoreFile,
		...
		data.moduleGlobals.tmpdir = outputTempFolder;
		_.extend(args, {
			PLAYER_ZIP: BIN_ROOT + '/' + (argv.inputZip ? argv.inputZip : 'iOS_Player.zip'),
			OUTPUT_ZIP: outputFolder + ((argv.outputZip) ? argv.outputZip : 'app.zip'),
			outputFolder: outputFolder,
			outputTempFolder: outputTempFolder
		});
		utility.writeJsonToStdout({
			"downloadFilePath": args.OUTPUT_ZIP
		});
	}
	...
	function loadConfigJson(argv) {
		...
		try {
			...
			var errMsg = "config_file could not be found : " + config_file_path;
			logger.fatal(errMsg);
			killProcess({
				err: "FATAL",
				msg: errMsg
...
function pluginsParse(plugins) {
	...
	if (!_.isUndefined(plugins)) {
		...
		_.each(plugins, function(item) {
			...
			res.push({
				name: plugin[0],
				path: convertAbsolute.convert(plugin[1])
			});
		});
	}
	return res;
