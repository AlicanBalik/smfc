...
function configUpdateFromConfigJson(config, args) {
	...
	try {
		...
		killProcess({
			err: 'AndroidManifest.xml error',
			msg: 'AndroidManifest.xml not found. Check your project folder',
...
function configUpdateFromArgs(config, args) {
	...
	_.extend(input, {
		...
		"apkTool": path.normalize(args.APK_TOOL),
		"plugins": args.PLUGINS,
		"inputApk": args.INPUT_APK,
		"inputApkx86": args.INPUT_APK_x86 || args.INPUT_APK.replace('.apk', '-x86.apk')
		// "inputApk": path.normalize(args.INPUT_APK),
		// "inputApkx86": path.normalize(args.INPUT_APK_x86) || path.normalize(args.INPUT_APK.replace('.apk', '-x86.apk'))
	});
	...
	_.extend(input.sign, {
		"inputApk": path.normalize(args.SIGN_INPUT_APK), // signer.
		"signer": path.normalize(args.SIGNER),
		"keystoreFile": path.normalize(args.KEYSTORE_FILE),
	...
	_.extend(input.java, {
		"path": path.normalize(args.JAVA),
		"maxMemory": args.MAX_JAVA_MEM
	});

	output.outputApk = path.normalize(args.OUTPUT_APK.replace(/smartface.apk/gm, input.license.name + '.apk'));
...
function configUpdateFoldersAndProfiles(config) {
	...
	if (!packageProfilesObj.isParsed()) {
		killProcess({
			...
			fullPath: path.normalize(root + '/Config/Android/PackageProfiles.xml')
		});
	}
